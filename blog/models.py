...
class Category(models.Model):
    ...
    slug = models.SlugField(unique=True)
    description = models.TextField('描述', max_length=240, default=settings.SITE_DESCRIPTION,
                                   help_text='用来作为SEO中description,长度参考SEO标准',null=True)
...
class Article(models.Model):
    ...
    def update_views(self):
        self.views += 1
        self.save(update_fields=['views'])#调用save告诉django更新view字段

    def get_pre(self):#前一篇文章
        return Article.objects.filter(id__lt=self.id).order_by('-id').first()

    def get_next(self):#后一篇
